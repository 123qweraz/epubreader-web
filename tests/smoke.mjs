...
`);
  ...
  const ap1 = await evalJs(`(() => {
    ...
    let gap23 = 0, rtAbove = false;
    if (toks.length >= 2) {
      const b = toks[1].getBoundingClientRect(), c = toks[2].getBoundingClientRect();
      gap23 = Math.round((c.left - (b.left + b.width)) * 10) / 10;
      const r1 = toks[1].querySelector("rt");
      if (r1) {
        const rb = toks[1].querySelector("rb") || toks[1];
        rtAbove = (r1.getBoundingClientRect().top + r1.getBoundingClientRect().bottom) / 2 < (rb.getBoundingClientRect().top + rb.getBoundingClientRect().bottom) / 2;
      }
    }
    return { embeddedRt, expect, hasTone: /[āáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜ]/.test(expect), tokGap23: gap23, nTok: toks.length, rtAbove, display: cur ? getComputedStyle(cur).display : "" };
  })()`);
  ...
  ok(ap1.rtAbove, `打字+注音并存时注音保持在字上方(rtAbove=${ap1.rtAbove}, display=${ap1.display})`);
  await evalJs(`document.getElementById("closeBookBtn").click()`);
  await sleep(300);
  await evalJs(`document.querySelector('#annotateSeg [data-ann="off"]').click()`);
  await sleep(300);
